// Fake but realistic personal finance data
window.LEDGER_DATA = (function () {
  const CATEGORIES = [
  ...
  function seed(s) { let x = s; return () => (x = (x * 9301 + 49297) % 233280) / 233280; }
  const rand = seed(7);
  ...
  const start = new Date(today); start.setMonth(start.getMonth() - 5);

  // Monthly recurring
  function addRecurring(months, day, account, category, merchant, amount, note) {
  ...
  for (let m = 0; m < 6; m++) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      TXNS.push({
        id: "t" + (id++),
        date: d.toISOString().slice(0, 10),
        account, category: cat, merchant,
        amount: amt, note: "", pending: false, recurring: false,
      });
    }
  }

  // A couple travel splurges
  ...
  // A few pending recent
  TXNS.push({ id: "t" + (id++), date: "2026-05-13", account: "credit",   category: "dining",    merchant: "Tartine Bakery", amount: -18.40, pending: true });
  TXNS.push({ id: "t" + (id++), date: "2026-05-12", account: "checking", category: "uncategorized", merchant: "SQ *MISSION CORNER", amount: -42.15, pending: true, note: "Needs review" });
  TXNS.push({ id: "t" + (id++), date: "2026-05-11", account: "amex",     category: "shopping",  merchant: "Amazon",         amount: -64.32, pending: true });
  ...
  // Sample CSV preview for the import flow
  ...
  return { CATEGORIES, ACCOUNTS, TXNS, BUDGETS, SAMPLE_CSV_ROWS, TODAY: today.toISOString().slice(0,10) };
})();
